...
class Qwen2Model(Qwen2PreTrainedModel):
    ...
    def make_4d_causal_mask(self, B, seq_length, inputs_embeds, img_start, img_end):
        # Create a lower triangular matrix of size (seq_length, seq_length)
        # with 1's in the lower triangle and 0's elsewhere
        mask = torch.tril(torch.ones((seq_length, seq_length), dtype=torch.bool, device=inputs_embeds.device))

        # Modify the mask to be bidirectional between img_start and img_end
        # Set all elements in this range to True (bidirectional)
        mask[img_start:img_end, img_start:img_end] = True

        # Expand this mask to the desired shape (B, 1, seq_len, seq_len)
        mask = mask.unsqueeze(0).unsqueeze(0).expand(B, 1, seq_length, seq_length)

        return mask

    def make_4d_causal_frame_wise_mask(self, B, seq_length, inputs_embeds, img_start, img_end, frame_length=210):
        # Create a lower triangular matrix of size (seq_length, seq_length)
        # with 1's in the lower triangle and 0's elsewhere
        num_frames = (img_end - img_start) // frame_length
        # print(num_frames * frame_length, img_end - img_start, img_end, img_start)
        assert int(num_frames * frame_length) == img_end - img_start, "img_end - img_start must be divisible by frame_length"

        mask = torch.tril(torch.ones((seq_length, seq_length), dtype=torch.bool, device=inputs_embeds.device))

        for t in range(num_frames):
            mask[img_start+t*frame_length:img_start+(t+1)*frame_length, img_start:img_start+(t+1)*frame_length] = True

        # Modify the mask to be bidirectional between img_start and img_end
        # Set all elements in this range to True (bidirectional)
        
        # mask[img_start:img_start+img_end, img_start:img_start+img_end] = True
    ...
    def forward(
        ...
        # print('position_ids:', position_ids[0,:20], position_ids[0,-20:])
...
class Qwen2ForCausalLM(Qwen2PreTrainedModel):
    ...
    def discrete_coords(self, world_coords, voxel_size=0.1):
    ...
    def correspondence_loss(self, features, voxel_ids):
        ...
        assert features.shape[0] == 32 * 14 * 14, "特征维度不匹配"
        ...
        return correspondence_loss.mean()
    ...
    def compute_correspondence(self, video_dict, hidden_states, voxel_size=0.1, img_pos_list=None, img_length_list=None):
        ...
        world_coords_discrete = self.discrete_coords(world_coords, voxel_size=voxel_size)
        ...
        loss = self.correspondence_loss(feature, keys_set[1])
        return loss
    ...
    def forward(
        ...
        if labels is not None:
            ...
            correspondence_loss = self.compute_correspondence(video_dict, hidden_states, voxel_size=0.1, img_pos_list=img_pos_list, img_length_list=img_length_list)
            loss += correspondence_loss
            print(f"correspondence_loss: {correspondence_loss.item()}")
